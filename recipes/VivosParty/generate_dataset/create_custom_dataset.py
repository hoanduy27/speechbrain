...
def parse_vivos_folder(vivos_folders):
    # parsing vivos
    # step 1:  we construct a dictionary with speakers ids
    utterances = []
    txt_files = []
    for vivos_dir in vivos_folders:
        utterances.extend(get_all_files(vivos_dir, match_and=[".wav"]))
        txt_files.extend(get_all_files(vivos_dir, match_and=["prompts.txt"]))
...
for indx, split in enumerate(["train", "dev", "eval"]):
    ...
    # we parse vivos utterances for current split
    c_vivos_folder = params["vivos_folders"][split]
    c_utterances, c_words = parse_vivos_folder(c_vivos_folder)
    create_metadata(
